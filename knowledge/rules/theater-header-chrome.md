...
**Desktop:** the dock (mic, camera, screen, invite, leave) sits in the **middle of the header**, not over the stage. The right side is **Settings only**. Theme and locale live in Settings, tab **General** (first). Tabs after that: Stage (admin), Devices, About last. Invite uses the device share sheet (`navigator.share`) with localized title + text that includes the room URL; clipboard is the fallback. General also offers **Install as app** while the session is not already a standalone PWA. See [pwa-and-native-share](/rules/pwa-and-native-share.md).
