...
export function BrandMark({ className }: { className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex size-10 overflow-hidden rounded-full border border-border bg-surface-sunken/80",
        className,
      )}
      aria-hidden="true"
    >
      <img src={logoMark} alt="" width={40} height={40} className="size-full object-cover" />
    </span>
  );
}
