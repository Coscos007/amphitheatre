...
import { BroadcastPane, hasBroadcastSurface } from "./broadcast-pane.tsx";
...
function autoGridClass(count: number): string {
...
export function StageGrid({ broadcast, ome, tiles, compact }: StageGridProps) {
  const { t } = useTranslation();
  const showBroadcast = hasBroadcastSurface(broadcast);
  ...
  const renderItem = (item: StageItem, featured: boolean) => (
    <StageTileChrome
      ...
      className={featured ? "h-full min-h-40" : compact ? "min-h-24" : "min-h-32"}
    >
  ...
  return (
    ...
    >
      <div className={cn("grid h-full min-h-0 flex-1 auto-rows-fr gap-3", autoGridClass(main.length))}>
        {main.map((item) => renderItem(item, true))}
      </div>
      {rail.length > 0 ? (
        <aside
          className={cn(
